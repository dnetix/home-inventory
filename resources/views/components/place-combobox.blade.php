{{-- Searchable location picker — same UX as the category combobox, since place
     trees grow too deep for a scroll-and-hunt sheet. Options come flattened
     from a PlaceTree; matching runs against the full breadcrumb, and the input
     shows the selected place's breadcrumb ("Garage › Shelf B"). --}}
...
@props(['placeIndex', 'property', 'nullLabel' => 'No location', 'live' => false])

@php
    $options = $placeIndex->flatten()->map(fn (array $entry) => [
        'id' => $entry['place']->id,
        'label' => $entry['place']->label,
        'depth' => $entry['depth'],
        'crumb' => implode(' › ', $placeIndex->breadcrumb($entry['place']->id)),
        'search' => mb_strtolower(implode(' ', $placeIndex->breadcrumb($entry['place']->id))),
    ])->values();
@endphp
...
<div class="relative" x-on:click.outside="close()" x-data="{
        ...
        label(id) { return this.options.find((option) => option.id === id)?.crumb ?? ''; },
    ...
    <div x-show="open" x-cloak
        ...
        <template x-for="option in filtered" :key="option.id">
            <button type="button" x-on:click="pick(option.id)"
                ...
                {{-- Indent by depth while browsing; searching flattens to breadcrumbs
                     because matched children often appear without their parents. --}}
                <span class="size-2 shrink-0 rounded-full bg-line-2"
                    :style="!search.trim() && `margin-left: ${option.depth * 14}px`"></span>
                <span class="flex-1 truncate text-[14px] font-semibold" :class="selected === option.id && 'text-accent-ink'"
                    x-text="search.trim() ? option.crumb : option.label"></span>
            </button>
        </template>
