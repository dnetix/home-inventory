...
- **Item status:** `items.status` — `ItemStatus` enum (`in_place`/`missing`/`broken`/`removed`), set via the status sheet on the item detail (mobile page + desktop pane, `ManagesItemActions::setStatus`). "Lent" stays **derived** from `activeLend`; a non-in-place status pill wins over the lent pill. `removed` behaves like a soft delete through the `notRemoved` global scope on `Item` — every list/search/stat/picker excludes removed items; `Item::withRemoved()` opts in (route binding and `Lend::item()`/`UpkeepTask::item()` already do, so removed items stay viewable/restorable and history keeps their names). The Delete button remains a hard delete.
